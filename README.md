...
**Business:**
1. Is `uid` generated by the source system or should the API generate it? What format is expected (UUID, slug, custom)?
2. What are the valid values for `type`? Should it be enforced as an enum?
3. Is `owner` a free-text string or a reference to a user/team in another system?

**Technical:**
4. What is the expected data volume? At what scale does in-memory storage need to be replaced with a database?
5. Should the API support updates (PUT/PATCH) and deletes? What are the rules around soft-delete vs hard-delete?
6. Is there an authentication/authorization requirement? Should certain teams only access their own assets?
